# Author: Ean Miller
# Program: Texas Roadhouse Bill Calculation System
# Purpose: This module calculates customer bills based on menu items ordered. It retrieves prices
# from the menu dictionary, supports multiples of the same item, ensures proper money formatting,
...
class OrderCalculator:
    """
    Handles all billing logic for a single order.
    Stores:
        - menu (dictionary of item → price)
        - order (dictionary of item → quantity)
    Provides:
    ...
    def add_item(self, item_name, quantity=1):
        """Adds an item to the order with validation."""
    ...
    def calculate_tax(self, rate=0.07):
        """Returns the tax amount based on subtotal."""
        return self.calculate_subtotal() * rate

    def calculate_tip(self, percentage):
        """Returns the tip amount based on subtotal + tax."""
        if percentage < 0:
    ...
    def calculate_total(self, tip_percentage):
        """Returns the final total including tax and tip."""
        return (
    ...
    def generate_receipt(self, tip_percentage):
        """Returns a formatted itemized receipt."""
        if not self.order:
